...
class MigrateBase(Command):
    ...
    def _get_migrations_list(self) -> List[dict]:
        ...
        """
        migrations_paths = app.config.get('ORDERED_MIGRATIONS_DIRS') or self._get_migrations_paths()
    ...
    def _get_migrations_paths() -> List[str]:
        ...
        for root, dirs, files in os.walk("."):
            if 'migrations' in dirs:
                ...
                elif '/shared_model' in root and app.config.get('SHARED_MODEL_USAGE_ENABLED'):
                    migrations_paths.append(
                        root.lstrip('./') + '/migrations')                    
                elif '/shared_model' not in root:
                    migrations_paths.append(
                        root.lstrip('./') + '/migrations')
    ...
    def run(self, name: Optional[str] = None) -> None:
        ...
        if not name:
            for migration in list_of_migrations:
                if migration['path'] != 'migrations.env': # exclude alembic migrations if exists
