...
database defaults to SQLite, which is not recommended for production usage.
...
* `metrics`: the metrics interface. (Not compatible with Unix sockets)
...
Connection settings for the manhole. Manhole sub-options include:
...
Server admins can configure custom templates for email content.
...
* `allow_unsafe_locale` is an option specific to Postgres. Under the default behavior, Synapse will refuse to
  ...
  by setting `allow_unsafe_locale` to true. Note that doing so may corrupt your database.
...
Settings for local room and user statistics collection. 
...
Unnecessary to set if using [`pusher_instances`](#pusher_instances) with `generic_workers`.
...
and email by running a `generic_worker` and adding it's [`worker_name`](#worker_name) to
...
Unnecessary to set if using [`federation_sender_instances`](#federation_sender_instances) with `generic_workers`.
...
by running a `generic_worker` and adding it's [`worker_name`](#worker_name) to
